...
export const explicitTerms = [
  ...
  /\bmad[ae]r\s*chod\b/i,
  /\bbehen\s*chod\b/i,
  /\bbhen\s*chod\b/i,
  /\bbahen\s*cho+d+h?\b/i,           // bahen choodh/chodh variants
  /(bhen|behen|bahen)\s*cho+d+h?\b/i, // captures bhen/behen/bahen choodh/chodh
  /\bbhosdi\s*ke\b/i,
  /\bbhosadike\b/i,
  /\bbhosri\s*w[ae]li[ye]?\b/i,
  /\bbhosdiw[ae]li[ye]?\b/i,
  /\bbhosri\s*ke\b/i,
  /\bbhai?nchod\b/i,
  /\bbhosda\b/i,
  /\bgaa+nd\b/i,
  ...
  /\bjhatu\b/i,
  /\bka+m\s*i+n[ae]\b/i,
  /\bharaami([yz]ad[ae])?\b/i,
  /\bkutt[iy][ae]?\b/i,
  /\bkutte\b/i,
  /\bkutte\s*ki\b/i,
  // Hinglish phrase slangs
  /(bhen|behen|bahen)\s*ki\s*ch[uo]{1,2}t{1,2}\b/i,
  /(bhen|behen|bahen)\s*ke\s*pakod(e|ey|e)?\b/i,
  /(bur|boor|bur)\s*ke\s*baa+l\b/i,           // bur ke baal
  /(maa|ma|ammi|mummy)\s*ki\s*ch[uo]{1,2}t{1,2}\b/i,
  /(maa|ma|ammi|mummy)\s*ke?\s*lod(e|a)?\b/i,
  /(bhen|behen|bahen)\s*ke?\s*lod(e|a)?\b/i,
  /(bhabhi|bhabi|bhabhiji)\s*ke?\s*(nudes?|bo{2}bs?|tits|ch[uo]{1,2}t{1,2})\b/i,
  /rand[iy]\s*ke?\s*(b[ea]cch[eaiy]|bache|bacha)\b/i,
  /(gaa?nd|gaand)\s*me?\s*(lund|l[ao]d[ae]|dand[ae])\b/i,
  /(suar|soo?ar|suwar|sooar|kutte?|kutte|gadhe?|gadha|ullu)\s*ki\s*jh?a+a?nt?\b/i,
  /(gaa?nd|gand)\s*me?\s*ke?e?d[ae]\b/i,
  /ch[uo]{1,2}t\s*me?\s*(mar(n[ae]|w[ae]|v[ao])|fad[eo]?)\b/i,
  /(lund|l[ao]d[ae])\s*ch(us|oos)[a-z]*\b/i,
  /(teri|tera|tumhari)\s*(maa|ma|ammi|mummy)\s*ki\s*ch[uo]{1,2}t{1,2}\b/i,
  /(teri|tera|tumhari)\s*(behen|bhen|bahen)\s*(ki|ke)\s*(ch[uo]{1,2}t{1,2}|l[ao]d[ea])\b/i,
  /(suar|soo?ar|suwar|sooar|kutte?)\s*ke\s*(b[ae]cch[ea]?|aulaad)\b/i,
  /(gaa?nd|gaand)\s*faa?d(du|d[eo]?)?\b/i,
  /(saali|saali)\s*rand[iy]\b/i,
  /\b(mkc|bkc)\b/i,
